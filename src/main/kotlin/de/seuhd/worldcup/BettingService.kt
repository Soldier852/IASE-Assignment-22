...
object BettingService {
    ...
    fun evaluateBonus(matches: List<Match>): Int {
        ...
        for (match in matches) {
            ...
            if (bet.prediction == Prediction.outcomeOf(home, away)) totalBonus++
            else continue

            if (bet.predictedHomeScore == home && bet.predictedAwayScore == away) totalBonus += 2
        }
        return totalBonus
    }
    ...
    fun changeBet(bet: Bet) {
        if (!bets.containsKey(bet.matchId)) throw IllegalArgumentException("Bet not in bets: $bets - $bet")
        bets[bet.matchId] = bet
    }
